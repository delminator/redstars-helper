...
machine de l'utilisateur. Le helper :
  1. décode le `nn:` hash → indices de palette (onnxruntime + le décodeur INT8 du site,
     bundlé : decoder.graph.onnx + decoder.int8 — les MÊMES fichiers que /models du site),
...
# Le décodeur est celui du SITE, octet pour octet : `decoder.graph.onnx` (le graphe, dont les
# poids sont des données externes) + `decoder.int8` (les poids, int8 par canal). Les mêmes
# fichiers que /models du frontend, déquantifiés par le même calcul (voir _dequantize) : la
# console décode donc exactement les indices que le navigateur affiche.
#
# (L'ancien `decoder.onnx` bundlé servait la page démo, avec des poids DIFFÉRENTS — ~5 % de
# concordance. Il a été retiré du helper en 0.5.64, avec la page.)
_MODEL_NAME = "decoder.graph.onnx"
_WEIGHTS_NAME = "decoder.int8"
_WEIGHTS = None        # les OrtValue doivent vivre aussi longtemps que la session


def _find_model():
    """Le graphe du décodeur, bundlé dans l'app Tauri, AVEC ses poids int8 à côté. Renvoie
    None si l'un des deux manque (repli texte, jamais de faux rendu)."""
    ...
    for pat in pats:
        for hit in sorted(glob.glob(pat)):
            if os.path.isfile(os.path.join(os.path.dirname(hit), _WEIGHTS_NAME)):
                return hit
    return None


def _dequantize(path):
    """Conteneur `decoder.int8` → {nom: poids float32}. Le calcul du moteur TS
    (packages/render/src/int8-weights.ts), à l'identique :
        int8     w = f16( float32(q) × float32(échelle[canal]) )
        float16  w = le demi-flottant décodé
    f16 = arrondi IEEE binary16, égalités au pair (le cast numpy). Format :
    'RSI8' · u32 version · u32 longueur d'entête · entête JSON · bourrage à 8 · données."""
    import json
    buf = open(path, "rb").read()
    if buf[:4] != b"RSI8" or int.from_bytes(buf[4:8], "little") != 1:
        raise ValueError("decoder.int8 : format inconnu")
    n = int.from_bytes(buf[8:12], "little")
    header = json.loads(buf[12:12 + n])
    base = -(-(12 + n) // 8) * 8
    out = {}
    for t in header["tensors"]:
        if t["dtype"] == "int8":
            q = _np.frombuffer(buf, _np.int8, t["len"], base + t["data"]).astype(_np.float32).reshape(t["shape"])
            s = _np.frombuffer(buf, "<f4", t["shape"][0], base + t["scales"]).reshape(-1, 1, 1, 1)
            out[t["name"]] = (q * s).astype(_np.float32).astype(_np.float16).astype(_np.float32)
        else:
            out[t["name"]] = _np.frombuffer(buf, "<f2", t["len"], base + t["data"]).astype(_np.float32).reshape(t["shape"])
    return out
...
def _session():
    ...
    try:
        ...
        w = _dequantize(os.path.join(os.path.dirname(model), _WEIGHTS_NAME))
        _WEIGHTS = [ort.OrtValue.ortvalue_from_numpy(w[k]) for k in w]
        so.add_external_initializers(list(w), _WEIGHTS)   # en mémoire : ni fichier temporaire, ni paquet onnx
        with open(model, "rb") as f:
            _SESSION = ort.InferenceSession(f.read(), so, providers=["CPUExecutionProvider"])
    except Exception:
        _SESSION = None
    return _SESSION
